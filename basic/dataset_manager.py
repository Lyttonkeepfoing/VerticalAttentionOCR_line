...
def pad_sequences_1D(data, padding_value):  # padding
    """
...
def pad_image_width_right(img, new_wigth, padding_value):
    """
    右侧width padding
    :param img:
    :param new_wigth:
    ...
    """
    h, w, c = img.shape
    pad_width = max((new_wigth - w), 0)   # 为什么要加max
    pad_right = np.ones((h, pad_width, c)) * padding_value   # 生成新的padding矩阵
    img = np.concatenate([img, pad_right], axis=1)  # concat img and new_padding
    return img


# print(np.ones((2, 2, 3))*0)



def pad_image_height_bottom(img, new_height, padding_value):
    """
    向下padding to a new height
    :param img:
    :param new_height:
    :param padding_value:
    :return:
...
class OCRCollateFunction:   # return formatted_batch_data
    """
    ...
    def __call__(self, batch_data):
        names = [batch_data[i]["name"] for i in range(len(batch_data))]  # 把name 取出来
        ids = [int(batch_data[i]["name"].split("/")[-1].split("_")[-1].split(".")[0]) for i in range(len(batch_data))]  # 把ids切出来

        labels = [batch_data[i]["token_label"] for i in range(len(batch_data))]  # 把token_label取出来
        labels = pad_sequences_1D(labels, padding_value=self.label_padding_value)  # padding
        labels = torch.tensor(labels).long()
        labels_len = [batch_data[i]["label_len"] for i in range(len(batch_data))] # 拿到labels的长度

        raw_labels = [batch_data[i]["label"] for i in range(len(batch_data))]  # 拿到batch_data中未处理的label
        unchanged_labels = [batch_data[i]["unchanged_label"] for i in range(len(batch_data))]
        nb_lines = [batch_data[i]["nb_lines"] for i in range(len(batch_data))]  # 拿到有多少行
        ...
        pad_line_token = list()  # pad_line_token 是一个空list
        line_len = [batch_data[i]["line_label_len"] for i in range(len(batch_data))]  # 每一行的line的长度
        for i in range(max(nb_lines)):  # 有多少行 循环多少次
            current_lines = [line_token[j][i] if i < nb_lines[j] else [self.label_padding_value] for j in range(len(batch_data))]
            pad_line_token.append(torch.tensor(pad_sequences_1D(current_lines, padding_value=self.label_padding_value)).long())  # current_line pad之后放到pad_line_token里面
            for j in range(len(batch_data)):  #
                if i >= nb_lines[j]:
                    line_len[j].append(0)
        """
zip(*) 和zip  zip() 函数用于将可迭代的对象作为参数，将对象中对应的元素打包成一个个元组，然后返回由这些元组组成的列表。
...
zip(*)函数利用 * 号操作符，可以将元组解压为列表。
        ...
        imgs = pad_images(imgs, padding_value=self.img_padding_value)   # padding img
        imgs = torch.tensor(imgs).float().permute(0, 3, 1, 2)  # 换一下维度顺序
...
class GenericDataset(Dataset):
    ...
    def __init__(self, params, set_name, custom_name, paths_and_sets, from_segmentation=False):
        ...
        if from_segmentation:  # TODO:什么是from seg
        ...
        self.curriculum_config = None  # TODO:什么是课程学习？？ 它是一种训练策略，模仿人类的学习过程，主张让模型先从容易的样本开始学习，并逐渐进阶到复杂的样本和知识
...
class DatasetManager:
    ...
    def __init__(self, params):  # params 传参进来
        self.params = params
    ...
    def load_datasets(self):
        self.train_dataset = self.dataset_class(self.params, "train", self.params["train"]["name"],
        ...
        for custom_name in self.params["valid"].keys():
            self.valid_datasets[custom_name] = self.dataset_class(self.params, "valid", custom_name,
                                                                  self.get_paths_and_sets("valid", self.params["valid"][custom_name]))
    ...
    def load_ddp_samplers(self):
        if self.params["use_ddp"]:
            self.train_sampler = DistributedSampler(self.train_dataset, num_replicas=self.params["num_gpu"],
                                                    rank=self.params["ddp_rank"], shuffle=True)
            for custom_name in self.valid_datasets.keys():
                self.valid_samplers[custom_name] = DistributedSampler(self.valid_datasets[custom_name],
